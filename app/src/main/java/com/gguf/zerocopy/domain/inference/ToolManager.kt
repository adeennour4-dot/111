...
class ToolManager {
  private val tools = mutableMapOf<String, (JSONObject) -> String>()
  ...
  fun registerTool(def: ToolDefinition, executor: (JSONObject) -> String) {
    tools[def.name] = executor
  }
  ...
  fun getToolDefinitionsJson(): String {
    val arr = JSONArray()
    for ((name, _) in tools) {
      val def = getToolDefinition(name) ?: continue
  ...
  private fun getToolDefinition(name: String): ToolDefinition? = when (name) {
    "get_current_time" -> ToolDefinition(
      name = "get_current_time",
      description = "Get current date and time",
      parameters = JSONObject()
    )
    "get_current_date" -> ToolDefinition(
      name = "get_current_date",
  ...
  fun parseToolCall(text: String): ToolCall? {
    // Try to find JSON tool call in the output
    // Format: {"name":"function_name","arguments":{...}}
    // Support both plain JSON and markdown code blocks
  ...
  fun executeTool(call: ToolCall): ToolResult {
    val executor = tools[call.name]
    val result = try {
      executor?.invoke(call.arguments) ?: "Tool '${call.name}' not found"
    } catch (e: Exception) {
      "Error: ${e.message}"
    }
  ...
  private fun parseExpression(expr: String): Double {
    ...
    if (pos < input.length) throw IllegalArgumentException("Unexpected character at position $pos")
    return result
  }
