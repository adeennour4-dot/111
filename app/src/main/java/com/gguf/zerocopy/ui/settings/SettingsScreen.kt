...
@Composable
fun SettingsScreen(onBack: () -> Unit, onThemeToggle: () -> Unit = {}) {
    ...
    Scaffold(
        ...
        Column(
            modifier = Modifier.padding(pad).padding(horizontal = 20.dp, vertical = 8.dp)
            ...
            Button(
                ...
                Text("Auto-Detect Device", color = ZcColors.Bg, fontWeight = FontWeight.Bold, fontSize = 13.sp)
            }

            Text("Context/GPU changes need model reload.", fontSize = 10.sp, color = ZcColors.Amber, fontFamily = FontFamily.Monospace)
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Switch(checked = lowRam, onCheckedChange = { lowRam = it }, colors = SwitchDefaults.colors(checkedTrackColor = ZcColors.Accent, checkedThumbColor = ZcColors.Bg))
            }
            Text("Reduces memory usage by limiting context and using 4-bit KV cache", fontSize = 9.sp, color = ZcColors.Text3, fontFamily = FontFamily.Monospace)

            OutlinedTextField(
                value = sysPrompt, onValueChange = { sysPrompt = it },
                modifier = Modifier.fillMaxWidth(), label = { Text("System Prompt", fontSize = 12.sp) }, maxLines = 4,
                shape = RoundedCornerShape(10.dp),
                colors = OutlinedTextFieldDefaults.colors(focusedBorderColor = ZcColors.Accent, unfocusedBorderColor = ZcColors.Border, focusedTextColor = ZcColors.Text, unfocusedTextColor = ZcColors.Text, cursorColor = ZcColors.Accent),
            ...
            Button(
                onClick = {
                    ...
                    scope.launch(Dispatchers.IO) {
                        ...
                        if (active != null && active.isModelLoaded) {
                            ...
                            benchResult = "Prefill: ${"%.1f".format(result.prefillTps)} t/s | Decode: ${"%.1f".format(result.decodeTps)} t/s"
                        } else {
                ...
                if (isBenchmarking) {
                    CircularProgressIndicator(modifier = Modifier.size(16.dp), color = ZcColors.Bg, strokeWidth = 2.dp)
            ...
            if (benchResult.isNotEmpty()) {
                Card(shape = RoundedCornerShape(10.dp), colors = CardDefaults.cardColors(containerColor = ZcColors.CardLight)) {
                    Text(benchResult, modifier = Modifier.padding(12.dp), fontSize = 11.sp, color = ZcColors.Text2, fontFamily = FontFamily.Monospace)
                }
            }
            ...
            OutlinedButton(
                ...
                modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(10.dp),
                colors = ButtonDefaults.outlinedButtonColors(contentColor = ZcColors.Accent)
            ) {
                Icon(Icons.Outlined.DarkMode, null, modifier = Modifier.size(16.dp))
                Spacer(Modifier.width(6.dp))
                Text("Toggle Theme", fontSize = 12.sp)
            }
