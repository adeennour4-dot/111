...
object SettingsManager {
    ...
    var systemPrompt: String
        get() = prefs?.getString("system_prompt", "You are a helpful, concise assistant running on-device. Respond clearly and directly.") ?: "You are a helpful, concise assistant running on-device. Respond clearly and directly."
    ...
    fun save(config: InferenceConfig, rp: RepeatPenaltyConfig) {
        nCtx = config.nCtx; nBatch = config.nBatch; maxTokens = config.maxNewTokens
        temperature = config.temperature; topP = config.topP; minP = config.minP
        gpuLayers = config.nGpuLayers; threads = config.nThreads
        lowRamMode = config.lowRamMode
        repeatPenalty = rp.repeatPenalty; freqPenalty = rp.freqPenalty; presPenalty = rp.presPenalty
    }
}
