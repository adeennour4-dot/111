...
static void finish_mnn_inference(const std::string& response_text, int tokens_generated) {
    g_history.emplace_back("assistant", response_text);
    {
        std::lock_guard<std::mutex> lock(g_mutex);
...
Java_com_gguf_zerocopy_domain_inference_MnnEngine_mnnExecuteInference(
    ...
    // Use response() with the actual prompt string so MNN tokenizes and
    // processes the real conversation. The previous code passed a hardcoded
    // placeholder token {1} instead of the prompt, so the model was always
    // primed on garbage input and produced empty/unrelated output regardless
    // of what the user typed.
    g_llm->response(full_prompt, nullptr, nullptr, 1);

    int tokens_generated = 0;
    std::string response_text;

    // Helper to process context after each step
    auto process_context = [&]() -> bool {
        std::lock_guard<std::mutex> lock(g_mutex);
        if (!g_llm) return true;
    ...
    // Process first response token
    tokens_generated = 1;
    g_tokens_generated = 1;
    if (process_context()) {
        finish_mnn_inference(response_text, tokens_generated);
        return;
    }

    // Stream remaining tokens
    for (int i = 1; i < g_cfg.max_new_tokens; i++) {
